...
#     Currently limited to just queue resources.
...
# Start of included rabbitmqadmin script.
# Alternatively, rename the script to give it a .py suffix and import conventionally
...
# end of included rabbitmqadmin script
...
class queue_service(service_base):
    ...
    def __init__(self, qs):
        super(queue_service, self).__init__('RMQ_' + qs['node'].split('@')[1] + '_Q_' + qs['name'])
        bqs = qs.get('backing_queue_status')
        self.metrics['wait_time'] = wait_time_metric(
            None if qs.get('head_message_timestamp') in ['', None] else (time() - qs['head_message_timestamp']),
            WAIT_TIME_WARN,
            WAIT_TIME_CRIT)
        ...
        self.metrics['msgs'] = msgs_metric(qs.get('messages_ready'))
        self.metrics['unacked'] = unacked_metric(qs.get('messages_unacknowledged'))

    def normal_output(self):
        s = 'OK - '
        s+= 'consumers {0}'.format(self.metrics['consumers'].to_description())  # NB consumers without a consumer (doing basic_get) won't appear 
